...
-- 평균 급여가 5000이상 부서 조회
-- WHERE과 HAVING의 차이
-- WHERE은 전체를 묶어주는 것 | HAVING은 묶어주고 싶은 것을 묶는 것
...
-- 학과 종류 검색
SELECT  DISTINCT STU_DEPT FROM TBL_STU ;

SELECT * FROM tbl_stu;

-- 각 학과별 학생 수 조회
SELECT DISTINCT STU_DEPT , COUNT(STU_ID)
FROM TBL_STU ts 
GROUP BY STU_DEPT;

SELECT * FROM tbl_stu;

-- 각 학과별 평균나이 조회 -- ROUND 함수 이용 자리수 구하는 것
SELECT DISTINCT STU_DEPT , ROUND(AVG(STU_AGE),2)
FROM TBL_STU ts 
GROUP BY STU_DEPT;
...
SELECT * FROM tbl_stu;
-- 학생 이름이 김으로 시작하거나 이로 시작하는 학생들의 학과별 수 조회
SELECT DISTINCT STU_DEPT, COUNT(STU_ID) 
FROM TBL_STU ts
...
SELECT * FROM tbl_stu;
-- 컴퓨터 공학과의 학생 중 25살 이상인 학생 수 조회
SELECT COUNT(STU_ID) 
FROM TBL_STU ts 
WHERE STU_DEPT LIKE '컴퓨터 공학과' AND STU_AGE >= 25;
